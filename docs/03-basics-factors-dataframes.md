...
\`\`\`{r, eval = FALSE, purl = FALSE} \## read in a CSV file and save it
as 'variants'
...
`<img src="../fig/rstudio_dataframeview.png" alt="rstudio data frame view" style="width: 1000px;"/>`{=html}
...
\`\`\`{r, purl = FALSE} \## get summary statistics on a data frame
...
\`\`\`{r, purl = FALSE} \## get the structure of a data frame
...
`{r, purl = FALSE} snps <- c(alt_alleles[alt_alleles=="A"],   alt_alleles[alt_alleles=="T"],   alt_alleles[alt_alleles=="G"],   alt_alleles[alt_alleles=="C"])`
...
`{r, purl = FALSE, fig.show='hide'} plot(snps)`
...
`{r, purl = FALSE} factor_snps <- factor(snps)`

Let's learn a little more about this new type of vector:

`{r, purl = FALSE} str(factor_snps)`
...
`{r, purl = FALSE} summary(factor_snps)`
...
> need to combine both (i.e. `table(as.factor(object))`) {: .callout}
...
`{r, purl = FALSE} ordered_factor_snps <- factor(factor_snps, levels = names(sort(table(factor_snps))))`
...
`{r, purl = FALSE} plot(ordered_factor_snps)`
...
> > ```
> >
> > e.  `{r, echo = TRUE, eval = FALSE} variants[-1, ]`
> >
> > `{r, echo = FALSE, eval = TRUE} head(variants[-1, ])`
> >
> > f.  
> >
> > ``` {r}
...
> > ```
> >
> > h.  `{r, echo = TRUE, eval = FALSE} variants[,c("sample_id")]`
> >
> > `{r, echo = FALSE, eval = TRUE} head(variants[,c("sample_id")])`
> >
> > i.  
> >
> > ``` {r}
...
> > ```
> >
> > k.  `{r, echo = TRUE, eval = FALSE} variants$sample_id`
> >
> > `{r, echo = FALSE, eval = TRUE} head(variants$sample_id)`
> >
> > l.  `{r, echo = TRUE, eval = FALSE} variants[variants$REF == "A",]`
> >
> > `{r, echo = FALSE, eval = TRUE} head(variants[variants$REF == "A",])`
> > {: .solution} {: .challenge}
...
\`\`\`{r, purl = FALSE} \# create a new data frame containing only
observations from SRR2584863
...
`{r, purl = FALSE} snp_chromosomes_2 <- c(3, 11, 'X', 6) typeof(snp_chromosomes_2) snp_chromosomes_2[1]`
...
`{r, purl = FALSE} snp_positions_2 <- c("8762685", "66560624", "67545785", "154039662") typeof(snp_positions_2) snp_positions_2[1]`

Now we can coerce `snp_positions_2` into a numeric type using
`as.numeric()`:

`{r, purl = FALSE} snp_positions_2 <- as.numeric(snp_positions_2) typeof(snp_positions_2) snp_positions_2[1]`

Sometimes coercion is straight forward, but what would happen if we
tried using `as.numeric()` on `snp_chromosomes_2`

`{r, purl = FALSE} snp_chromosomes_2 <- as.numeric(snp_chromosomes_2)`

If we check, we will see that an `NA` value (R's default value for
missing data) has been introduced.

`{r, purl = FALSE} snp_chromosomes_2`
...
`{r, purl = FALSE} as.numeric(variants$sample_id)`
...
\`\`\`{r, purl = FALSE} \# make the 'REF' column a character type column
...
`{r, purl = FALSE} sorted_by_DP <- variants[order(variants$DP), ] head(sorted_by_DP$DP)`
...
> > ## Solution
> >
> > `{r, purl = FALSE}    sorted_by_DP <- variants[order(variants$DP, decreasing = TRUE), ]    head(sorted_by_DP$DP)`
> > {: .solution} {: .challenge}

You can rename columns:

\`\`\`{r, purl = FALSE} colnames(variants)\[colnames(variants) ==
"sample_id"\] \<- "strain"
...
`<img src="../fig/rstudio_import_menu.png " alt="rstudio import menu" style="width: 600px;"/>`{=html}

Next, under **File/Url:** click the `<KBD>`{=html}Browse`</KBD>`{=html}
...
`<img src="../fig/rstudio_import_screen.png " alt="rstudio import screen" style="width: 1200px;"/>`{=html}
...
Click the `<KBD>`{=html}Import`</KBD>`{=html} button to import the data.

Finally, let's check the first few lines of the `Ecoli_metadata` data
frame:

`{r, echo = FALSE, eval = TRUE, purl = FALSE, warning = FALSE} ## silently read in CSV file from CyVerse download.file("https://de.cyverse.org/dl/d/17FAE062-2673-417B-8544-77CC46974A37/Ecoli_metadata.xlsx",               destfile = "Ecoli_metadata.xlsx") Ecoli_metadata <- readxl::read_xlsx("Ecoli_metadata.xlsx")`

`{r, purl = FALSE} head(Ecoli_metadata)`
...
>
> B)  What are categories are there in the `cit` column? *hint*: treat
>     column as factor
>
...
>
> G)  Create a new column (name genome_size_bp) and set it equal to the
>     genome_size multiplied by 1,000,000
>
> H)  Save the edited Ecoli_metadata data frame as
>     "exercise_solution.csv" in your current working directory.
>
> > ## Solution
> >
> > `{r, purl = FALSE} dim(Ecoli_metadata) levels(as.factor(Ecoli_metadata$cit)) table(as.factor(Ecoli_metadata$cit)) Ecoli_metadata[7,7] median(Ecoli_metadata$genome_size) colnames(Ecoli_metadata)[colnames(Ecoli_metadata) == "sample"] <- "sample_id" Ecoli_metadata$genome_size_bp <- Ecoli_metadata$genome_size * 1000000 write.csv(Ecoli_metadata, file = "exercise_solution.csv")`
> > {: .solution} {: .challenge}
