...
most common modes you will encounter in R are:
  ...
  (abbreviation)    
  ----------------- -----------------------------------------------------
  Numeric (num)     Numbers such floating point/decimals (1.0, 0.5,
                    3.14), there are also more specific numeric types
                    (dbl - Double, int - Integer). These differences are
                    not relevant for most beginners and pertain to how
                    these values are stored in memory

  Character (chr)   A sequence of letters/numbers in single '' or double
                    " " quotes
...
`{r, purl = FALSE} (1 + (5 ** 0.5))/2`

and importantly, can be used on any object that evaluates to
(i.e. interpreted by R) a numeric object:

`{r, echo = FALSE, purl = FALSE} human_chr_number <- 23`

\`\`\`{r, purl = FALSE} \# multiply the object 'human_chr_number' by 2
...
`{r, purl = FALSE} # Check the mode, length, and structure of 'snp_genes' mode(snp_genes) length(snp_genes) str(snp_genes)`
...
\`\`\`{r, purl = FALSE} \# Some interesting human SNPs \# while accuracy
is important, typos in the data won't hurt you here

snps \<- c("rs53576", "rs1815739", "rs6152", "rs1799971")
snp_chromosomes \<- c("3", "11", "X", "6") snp_positions \<- c(8762685,
66560624, 67545785, 154039662)
...
\`\`\`{r, purl = FALSE} \# get the 1st through 3rd value in the snp
vector
...
`{r, purl = FALSE} # get the 1st through the 3rd value, and 4th value in the snp vector # yes, this is a little silly in a vector of only 4 values. snp[c(1:3,4)]`
...
`{r, purl = FALSE} snp_positions[snp_positions > 100000000]`
...
>
> `{r, purl = FALSE} snp_positions > 100000000`
>
...
> true values:
>
> `{r, purl = FALSE} snp_positions[c(FALSE, FALSE, FALSE, TRUE)]`
>
...
>
> `{r, purl = FALSE} which(snp_positions > 100000000)`
>
...
>
> `{r, purl = FALSE} snp_marker_cutoff <- 100000000 snp_positions[snp_positions > snp_marker_cutoff]`
>
...
\`\`\`{r, purl = FALSE} \# current value of 'snp_genes': \# chr \[1:7\]
"OXTR" "ACTN3" "AR" "OPRM1" "CYP1A1" NA "APOA5"
...
> > ## Solution
> >
> > `{r, purl = FALSE} mode(snps) mode(snp_chromosomes) mode(snp_positions)`
> > {: .solution} {: .challenge}
...
> > ## Solution
> >
> > `{r, purl = FALSE} snps <- c(snps, "rs662799") snps snp_chromosomes <- c(snp_chromosomes, "11") # did you use quotes? snp_chromosomes snp_positions <- c(snp_positions, 116792991) snp_positions`
> > {: .solution} {: .challenge}
...
> > ## Solution
> >
> > `{r, purl = FALSE} snp_genes <- snp_genes[-5] snp_genes <- c(snp_genes, NA, NA) snp_genes`
> > {: .solution} {: .challenge}

> ## Review Exercise 4
>
> Using indexing, create a new vector named `combined` that contains: -
> The the 1st value in `snp_genes` - The 1st value in `snps` - The 1st
> value in `snp_chromosomes` - The 1st value in `snp_positions`
>
> > ## Solution
> >
> > `{r, purl = FALSE} combined <- c(snp_genes[1], snps[1], snp_chromosomes[1], snp_positions[1]) combined`
> > {: .solution} {: .challenge}
...
> > ## Solution
> >
> > `{r, purl = FALSE} typeof(combined)` {: .solution} {: .challenge}
